...
class Pore3D: public Pore {
    ...
    private:
        ...
        std::vector<containers::Tuple4D<double> > pore_family_;
    
    public:
        Pore3D() = default;
        Pore3D(std::tuple<double, double, double> coordinates, int id, double radius, 
            std::set<int> connections, std::vector<containers::Tuple4D<double> > pore_family) {
                      id_ = id;
                      radius_ = radius;
        ...
        std::vector<containers::Tuple4D<double> > getPoreFamily() const {return pore_family_;}
